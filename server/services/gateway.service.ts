...
import { APP_CONSUL_PROVIDER } from 'server/core'
import { ConsulService } from 'server/modules/consul/services/consul.service'
...
export class GatewayService {
  ...
  public findService({ serviceName = undefined, serviceTags = [] }) {
    ...
    // 通过Name查找Service
    const findServiceByName = (service) => isNullOrUndefined(serviceName) || service.Service === serviceName

    return Object.values(this.services).find((service) => findServiceByTag(service) && findServiceByName(service))
  }
